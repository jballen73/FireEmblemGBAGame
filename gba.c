...
void setPixel(int x, int y, u16 color) {
    // TA-TODO: IMPLEMENT
    videoBuffer[WIDTH*y + x] = color;
}

void drawRectDMA(int x, int y, int width, int height, volatile u16 color) {
    // TA-TODO: IMPLEMENT
    volatile u16 fillColor = color;
    for (int i = 0; i < height; i++) {
        DMA[3].dst = (videoBuffer + ((WIDTH*(y + i)) + x ));
        DMA[3].src = &fillColor;
        DMA[3].cnt = width | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT | DMA_NOW | DMA_ON;
    }
}
...
void drawImageDMA(int x, int y, int width, int height, const u16 *image) {
    ...
    for (int i = 0; i < height; i++) {
        ...
        DMA[3].cnt = width | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_NOW | DMA_ON;
    }
}
...
void fillScreenDMA(volatile u16 color) {
    ...
    DMA[3].src = &fillColor;
    DMA[3].dst = videoBuffer;
    DMA[3].cnt = (WIDTH*HEIGHT) | DMA_DESTINATION_INCREMENT | DMA_SOURCE_FIXED | DMA_NOW | DMA_ON;
}
